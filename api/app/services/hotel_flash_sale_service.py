...
class HotelFlashSaleService:
    ...
    @staticmethod
    async def list_hotel_flash_sales(query: Dict) -> List[Dict]:
        """活動查詢"""
        ...
        if active_only:
            # 只查詢活躍的活動
            sales = await HotelFlashSale.find(
                HotelFlashSale.is_active == True,
                HotelFlashSale.start_time <= now,
                HotelFlashSale.end_time >= now
            ).sort(HotelFlashSale.start_time).to_list()
        else:
            sales = await HotelFlashSale.find_all().sort(HotelFlashSale.start_time).to_list()
    ...
    @staticmethod
    async def create_hotel_flash_sale(data: Dict) -> Dict:
        """新增活動（同時生成每日庫存）"""
        ...
        # 生成每日庫存
        await HotelFlashSaleService._generate_daily_inventory(new_sale)

        # 返回序列化的数据
        return {
            "id": str(new_sale.id),
            "title": new_sale.title,
            "hotelId": str(new_sale.hotel_id.ref.id) if hasattr(new_sale.hotel_id, 'ref') else str(new_sale.hotel_id),
            "roomId": str(new_sale.room_id.ref.id) if hasattr(new_sale.room_id, 'ref') else str(new_sale.room_id),
    ...
    @staticmethod
    async def _generate_daily_inventory(sale: HotelFlashSale) -> None:
        """生成每日库存"""
        ...
        if inventories:
            await HotelFlashSaleInventory.insert_many(inventories)
    ...
    async def update_hotel_flash_sale(sale_id: str, data: Dict) -> Dict:
        ...
        return {
            ...
            "bannerUrl": updated_sale.banner_url,
    ...
    @staticmethod
    async def get_all_hotel_flash_sale_orders() -> List[Dict]:
        """後台查全部訂單"""
        orders = await HotelFlashSaleOrder.find_all().sort("-created_at").to_list()
        ...
        for order in orders:
            order_data = {
                "id": str(order.id),
                "saleId": str(order.sale_id.ref.id) if hasattr(order.sale_id, 'ref') else str(order.sale_id),
                "userId": str(order.user_id.ref.id) if hasattr(order.user_id, 'ref') else str(order.user_id),
                "hotelId": str(order.hotel_id.ref.id) if hasattr(order.hotel_id, 'ref') else str(order.hotel_id),
                "roomId": str(order.room_id.ref.id) if hasattr(order.room_id, 'ref') else str(order.room_id),
                "date": order.date,
